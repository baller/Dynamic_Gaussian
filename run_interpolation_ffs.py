...
基于 run_interpolation.py，替换深度估计为 Fast-FoundationStereo：
  - 使用 RtStereoHumanModel (depth_mode='ffs') 加载模型
  - get_item_free 保留 Tf_x / ref_intr（FFS 需要用于主点偏移补偿），
    但不需要 flow_init
  - 使用原版 pts2render 完成高斯渲染
  - 输出 RGB 帧、逆深度伪彩图，并自动合成视频

用法:
    python run_interpolation_ffs.py -i s1a6 \\
        --ckpt experiments/gps_plus_ffs_0312/ckpt/gps_plus_latest.pth \\
...
from lib.human_loader import load_json_to_np, stereo_depth2flow
...
class FFSRenderModel(nn.Module):
    """
    GPS+ (FFS) 自由视角渲染器。

    与原版 StereoHumanModel 的关键区别：
    - get_item_free 保留 Tf_x / ref_intr（FFS 需要做主点偏移补偿）
    - 不需要 flow_init（FFS 不用 RAFT-Stereo 的光流初始化）
    """
...
def inv_depth_to_colormap(inv_depth_tensor, mask=None):
    """逆深度 [B,1,H,W] → JET 伪彩色 numpy [H,W,3] BGR"""
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='GPS+ (FFS) 自由视角插值渲染')
    parser.add_argument('-i', '--input', type=str, required=True,
                        help='目标序列名称（如 s1a6）')
    parser.add_argument('--ckpt', type=str, required=True,
                        help='GPS+ (FFS) checkpoint 路径')
    ...
    for pair, sid in zip(cam_pairs, sid_list):
        ...
        renderer = FFSRenderModel(cfg, arg.ckpt, novel_extrs, novel_intrs, tar_n, sid)
