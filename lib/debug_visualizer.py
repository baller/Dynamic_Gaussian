...
class DebugVisualizer:
    ...
    def _process_tensor(
        self, tensor: Union[torch.Tensor, np.ndarray], normalize: bool = True
    ) -> np.ndarray:
        """
    ...
    def show_tensor(
        ...
        if colormap and HAS_MATPLOTLIB:
            ...
            filepath = self._generate_filename(name)
            cv2.imwrite(str(filepath), img[:, :, ::-1])
    ...
    def show_training_data(
        ...
        if "lmain" in data:
            ...
            if "depth" in data["lmain"]:
                path = self.show_tensor(
                    data["lmain"]["depth"], f"L_depth_{step_str}", colormap="jet"
                )
        ...
        if "rmain" in data:
            ...
            if "depth" in data["rmain"]:
                path = self.show_tensor(
                    data["rmain"]["depth"], f"R_depth_{step_str}", colormap="jet"
                )
        ...
        if images:
            grid_path = self.show_image_grid(
                images, names, f"training_data_{step_str}"
            )
    ...
    def show_gaussians_info(
        self, data: Dict[str, Any], view: str = "lmain", step: Optional[int] = None
    ) -> Dict[str, str]:
        """
        显示高斯点相关信息
        ...
        if "rgb" in view_data:
            path = self.show_tensor(view_data["rgb"], f"{view}_rgb_{step_str}")
            saved_paths["rgb"] = path

        if "opacity" in view_data:
            path = self.show_tensor(
                view_data["opacity"], f"{view}_opacity_{step_str}", colormap="viridis"
            )
...
def show_depth(depth: Union[torch.Tensor, np.ndarray], name: str = "depth") -> str:
    """快速显示深度图的便捷函数"""
